...
import { 
  ...
  Activity, Layers, Check, ChevronUp
} from 'lucide-react';
...
export const OperationsHub: React.FC<OperationsHubProps> = ({ 
  ...
  // ... (Other helper functions mostly same, omitted for brevity but preserved in structure)
                      ...
                      return (
                          <React.Fragment key={task.id}>
                              ...
                              {isExpanded && task.checklist.map((item, idx) => {
                                  // Mock visualization: distribute items across parent duration
  ...
  if (viewLevel === 'project-board' && activeProject && activeOrg) {
      return (
                                  ...
                                  return (
                                      <div key={status} className="flex-1 min-w-[280px] bg-white/5 rounded-xl border border-white/10 flex flex-col h-full" onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, status)}>
                                          ...
                                          <div className="p-3 flex-1 overflow-y-auto custom-scrollbar">
                                              ...
                                              <button onClick={() => {
                                                    const newTask: Task = { id: `t-${Date.now()}`, projectId: activeProject.id, clientId: activeOrg.id, title: 'New Task', description: '', status: status as TaskStatus, assignee: 'Unassigned', dueDate: new Date().toISOString().split('T')[0], priority: 'Medium', checklist: [] };
          ...
          {/* Edit Project Modal and other popups remain... */}
          {editingProject && (
              <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                  <div className="w-full max-w-2xl bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                       <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
                           <h2 className="text-xl font-headline text-white font-bold">Edit Project Details</h2>
                           <button onClick={() => setEditingProject(null)}><X className="w-6 h-6 text-slate-500 hover:text-white"/></button>
                       </div>
                       <div className="p-6 overflow-y-auto space-y-4">
                           <div>
                               <label className="text-xs text-slate-400 uppercase font-bold">Project Title</label>
                               <input value={editingProject.title} onChange={e => setEditingProject({...editingProject, title: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded p-2 text-white mt-1"/>
                           </div>
                           <div>
                               <label className="text-xs text-slate-400 uppercase font-bold">Description</label>
                               <textarea value={editingProject.description} onChange={e => setEditingProject({...editingProject, description: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded p-2 text-white mt-1 h-20"/>
                           </div>
                           {/* ... rest of fields ... */}
                           <div className="grid grid-cols-2 gap-4">
                               <div>
                                   <label className="text-xs text-slate-400 uppercase font-bold">Due Date</label>
                                   <input type="date" value={editingProject.dueDate} onChange={e => setEditingProject({...editingProject, dueDate: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded p-2 text-white mt-1"/>
                               </div>
                               <div>
                                   <label className="text-xs text-slate-400 uppercase font-bold">Status</label>
                                   <select value={editingProject.status} onChange={e => setEditingProject({...editingProject, status: e.target.value as any})} className="w-full bg-[#0f172a] border border-white/10 rounded p-2 text-white mt-1">
                           ...
                           <div className="pt-4 border-t border-white/10 flex flex-wrap justify-between items-center gap-2">
                               <div className="flex gap-2 w-full md:w-auto">
                                   <button onClick={() => setEditingProject({...editingProject, isArchived: !editingProject.isArchived})} className={`flex-1 md:flex-none px-3 py-1 text-sm rounded border ${editingProject.isArchived ? 'bg-orange-500/20 text-orange-400 border-orange-500/50' : 'bg-white/5 text-slate-400 border-white/10'}`}>{editingProject.isArchived ? 'Unarchive Project' : 'Archive Project'}</button>
                                   <button onClick={(e) => deleteProject(editingProject.id, e)} className="flex-1 md:flex-none px-3 py-1 text-sm rounded border border-red-500/20 bg-red-500/10 text-red-400 hover:bg-red-500/20">Delete Project</button>
                               </div>
                           </div>
                       </div>
          ...
          {isManageProjectMembersOpen && activeProject && (
              <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                  <div className="w-full max-w-lg bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
                      <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
                          <div>
                              <h2 className="text-xl font-headline text-white font-bold">Manage Project Team</h2>
                              <p className="text-xs text-slate-400">Add/Remove members for "{activeProject.title}"</p>
                          </div>
                          <button onClick={() => setIsManageProjectMembersOpen(false)}><X className="w-6 h-6 text-slate-500 hover:text-white"/></button>
                      </div>
                          ...
                          <div>
                              ...
                              <div className="space-y-2">
                                  {teamMembers
                                    ...
                                    .map(m => {
                                        const isMember = activeProject.members.includes(m.name);
                                        return (
                                            <div 
                                                key={m.id} 
                                                onClick={() => toggleProjectMember(m.name)}
                                                className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors ${isMember ? 'bg-white/10 border-white/20 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'}`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold">{m.name.charAt(0)}</div>
                                                    <div>
                              ...
                              <div className="space-y-2">
                                  {activeOrg.users.map(u => {
                                        ...
                                        return (
                                            <div 
                                                key={u.id} 
                                                onClick={() => toggleProjectMember(u.name)}
                                                className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors ${isMember ? 'bg-white/10 border-white/20 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'}`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-full bg-indigo-900/50 text-indigo-200 flex items-center justify-center text-xs font-bold">{u.name.charAt(0)}</div>
                                                    <div>
  ...
  if (viewLevel === 'org-detail' && activeOrg) {
      // ... (Rest of Org Detail View is largely unchanged, but just returning structure to ensure consistent file write)
          ...
          <div className="h-full flex flex-col animate-in slide-in-from-right-8 duration-500 relative z-0">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-white/10 pb-6 gap-4">
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          <h2 className="text-3xl font-headline font-bold text-white mb-1">{activeOrg.name}</h2>
                          <div className="flex items-center gap-4 text-sm text-slate-400"><span className="flex items-center"><Briefcase className="w-4 h-4 mr-1"/> {activeOrg.industry}</span><span className="w-1 h-1 rounded-full bg-slate-600"></span><span className="flex items-center"><Users className="w-4 h-4 mr-1"/> {activeOrg.users.length} Users</span></div>
                      </div>
                  </div>
              ...
              {/* Project Modal (Updated with Client Selection) */}
              {isProjectModalOpen && (
                  <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                      <div className="w-full max-w-2xl bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                           <div className="p-6 border-b border-white/10 flex justify-between items-center bg-black/20">
                               <h2 className="text-xl font-headline text-white font-bold">Initialize New Project</h2>
                               <button onClick={() => setIsProjectModalOpen(false)}><X className="w-6 h-6 text-slate-500 hover:text-white"/></button>
                           </div>
                                   ...
                                   <>
                                       ...
                                       <div className="grid grid-cols-2 gap-4">
                                           ...
                                           <div>
                                               ...
                                               <div className="flex flex-wrap gap-2 mt-1 bg-white/5 p-2 rounded min-h-[42px]">
                                                   {/* Employees */}
                                                   {teamMembers.filter(m => m.role !== UserRole.SUPER_ADMIN && activeOrg.assignedEmployees?.includes(m.id)).map(m => (
                                                       <button 
                                                           key={m.id} 
                                                           onClick={() => { 
                                                   ...
                                                   {activeOrg.users.map(u => (
                                                       <button 
                                                           ...
                                                           onClick={() => { 
                                                               ...
                                                               setNewProjectForm({ ...newProjectForm, members: exists ? newProjectForm.members.filter(n => n !== u.name) : [...newProjectForm.members, u.name] }); 
                                                           }} 
                                                           className={`text-xs px-2 py-1 rounded border ${newProjectForm.members.includes(u.name) ? 'bg-indigo-500 text-white border-indigo-500' : 'border-slate-600 text-slate-400'}`}
                                                       >
                                                           {u.name} (Client)
                                                       </button>
                                                   ))}
              ...
              {editingProject && (
                  <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                      <div className="w-full max-w-2xl bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                           ...
                           {/* ... Content same as other edit modal, using editingProject state ... */}
                           <div className="p-6 overflow-y-auto space-y-4">
  ...
  return (
      <div className="h-full flex flex-col animate-in fade-in">
          ...
          {/* New Global Operations State Summary Bar */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div className="glass-panel p-4 rounded-xl border border-white/10 flex items-center justify-between">
                  <div>
                      <div className="text-slate-400 text-xs uppercase tracking-wider mb-1">Total Active Projects</div>
                      <div className="text-2xl font-bold text-white font-headline">
                          {projects.filter(p => p.status === 'Active' && !p.isArchived).length}
                      </div>
                  </div>
              ...
              <div className="glass-panel p-4 rounded-xl border border-white/10 flex items-center justify-between">
                  <div>
                      <div className="text-slate-400 text-xs uppercase tracking-wider mb-1">Global Tasks Pending</div>
                      <div className="text-2xl font-bold text-white font-headline">
                          {tasks.filter(t => t.status !== TaskStatus.DONE && !t.isArchived).length}
                      </div>
                  </div>
              ...
              <div className="glass-panel p-4 rounded-xl border border-white/10 flex items-center justify-between">
                  <div>
                      <div className="text-slate-400 text-xs uppercase tracking-wider mb-1">Organizations</div>
                      <div className="text-2xl font-bold text-white font-headline">
              ...
              {visibleOrganizations.map(org => {
                  ...
                  return (
                      ...
                      >
                          ...
                          <h3 className="text-xl font-bold text-white mb-1 font-headline">{org.name}</h3>
                          <p className="text-sm text-slate-500 mb-6 font-body">{org.industry}</p>
                          
                          <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
                              <div>
                                  <div className="text-2xl font-display text-white">{activeProjCount}</div>
                                  <div className="text-xs text-slate-500 uppercase tracking-wider">Active Projects</div>
                              </div>
                              <div>
                                  <div className="text-2xl font-display text-white">{org.users.length}</div>
